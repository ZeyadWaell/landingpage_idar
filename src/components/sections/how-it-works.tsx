    ...
    <Section id="how-it-works" className="bg-[#eff4ff]/40 dark:bg-white/[0.02]">
      <Reveal className="mx-auto max-w-2xl text-center">
        ...
        <h2 className="mt-3 text-3xl font-bold tracking-tight text-[#0a0a0a] sm:text-4xl dark:text-white">
          {h.title}{" "}
          <span className="text-[#0040c1] dark:text-[#699aff]">{h.titleHighlight}</span>
        </h2>
        <p className="mt-4 text-lg text-[#171717]/70 dark:text-[#eff4ff]/70">{h.subtitle}</p>
      </Reveal>

      <RevealGroup as="ol" className="mt-16 grid gap-8 lg:grid-cols-3">
        {h.steps.map((item, index) => (
          <RevealItem as="li" key={item.step} className="relative">
            ...
            <h3 className="mt-1 text-xl font-semibold text-[#0a0a0a] dark:text-white">{item.title}</h3>
